...
void AudioDevice::removePropertyListener(AudioObjectPropertySelector selector,
                                         AudioObjectPropertyScope scope,
                                         AudioObjectPropertyElement element,
                                         AudioObjectPropertyListenerProc proc,
                                         void *clientData) {
    ...
    if (err != noErr) {
        syslog(LOG_WARNING,
               "ProxyAudio: failed to remove listener for '%c%c%c%c' '%c%c%c%c' %u",
               (listenerPropertyAddress.mSelector >> 24) % 0xFF,
               (listenerPropertyAddress.mSelector >> 16) % 0xFF,
               (listenerPropertyAddress.mSelector >> 8) % 0xFF,
               (listenerPropertyAddress.mSelector) % 0xFF,
...
void AudioDevice::setupIOProc(AudioDeviceIOProc inProc, void *clientData) {
    ...
    if (err != noErr) {
        syslog(LOG_WARNING, "ProxyAudio: error: failed to setup proc id on device %u with status %d", id, err);
        procId = nullptr;
    }
}
...
void AudioDevice::start() {
    ...
    if (err != noErr) {
        syslog(LOG_WARNING, "ProxyAudio: error: failed to start device %u with status %d", id, err);
    } else {
        isStarted = true;
    }
}
...
AudioDeviceID AudioDevice::audioDeviceIDForDeviceUID(CFStringRef uid) {
    AudioObjectID result = audioDeviceIDForUID(uid, kAudioHardwarePropertyTranslateUIDToDevice);
